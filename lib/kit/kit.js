var Kit = function(args){
	...
	if(tag && tag.open && tag.close && tag.open.tag in self.plugin_info.tags){
	    ...
	    var plugin_name = self.plugin_info.tags[tag.open.tag].plugin_name;
	    var plugin = self.plugin_info.plugins[plugin_name];
	    var id = -1;
	    matches = cm.getRange(tag.open.from, tag.open.to).match(/id="([0-9]+)"/);
	    console.log("MM",matches,cm.getRange(tag.open.from, tag.open.to));
	    if(matches){
		...
		if(id >= 0 && !self.plugin_info.tags[tag.open.tag].text){
		    plugin.cleanup(id);
		}
	    }
		...
		console.log("ASD",p);
	    	if(change.update) change.update(tag.open.to, tag.close.from, plugin.edit(self.editor, change, id, tag.open.to, tag.close.from));
	    }
	    else if(in_tag){
		change.cancel();
	    }
...
Kit.prototype.get_data = function(){
	...
	console.log(plugin,plugin.properties.text);
	if(!plugin.properties.text){
		...
		var s = plugin.serialize(nodes[0].cloneNode(true));
...
Kit.prototype.set_doc = function(doc){
	...
	cur.findNext()
	console.log("CUR",cur);
	docs[d].plugin.deserialize(this.editor, docs[d].doc, cur.from().line-1);
	cur.replace("");
    }
}
