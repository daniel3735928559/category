(function (Vue, Router, Vuex, CodeMirror) {
  ...
  var store = new Vuex__default.Store({
  	...
  	sorted: state => nodelist => {
  		...
  		if('index' in state.nodes[a] && 'index' in state.nodes[b]) {
  		    return parseInt(state.nodes[b].index) - parseInt(state.nodes[a].index);
  		}
  ...
  //
   ...
   var script$3 = {
       ...
       computed: {
  	 label_neighbours: function(node, label) {
  	     var ans = [];
  	     for(var m in this.nodes[node].edges[this.mode == 'menu' ? 'has' : 'is'][label]) {
  ...
  var __vue_render__$4 = function() {
    ...
    return _c("div", { staticClass: "label_index" }, [
      _vm.mode == "by" || _vm.mode == "menu"
        ? _c(
            "ul",
            _vm._l(_vm.sorted(_vm.headers), function(n) {
              return _c(
                "li",
                [
                  _c(
                  ...
                  _c(
                    "ul",
                    _vm._l(
                      _vm.sorted(_vm.label_neighbours(n, _vm.label)),
                      function(m) {
                        return _c(
                          "li",
                          [
